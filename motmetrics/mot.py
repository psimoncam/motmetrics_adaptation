        ...
        if oids.size * hids.size > 0:
            ...
            for i in range(oids.shape[0]):
                ...
                if np.isfinite(dists[i, j]):
                    ...
                    if o not in self.tracks_per_id:
                        self.tracks_per_id[o] = []
                    if h not in self.tracks_per_id[o]:
                        self.tracks_per_id[o].append(h)
                        
                    if h not in self.ids_per_track:
                        self.ids_per_track[h] = []
                    if o not in self.ids_per_track[h]:
                        self.ids_per_track[h].append(o)
            ...
            for i, j in zip(rids, cids):
                ...
                if cat1 == 'SWITCH':
                    if h not in self.hypHistory:
                        ...
                        if oids[i] not in self.ascend_dict:
                            self.ascend_dict[oids[i]] = 0
                        self.ascend_dict[oids[i]] += 1
                    if oids[i] not in self.switches_dict:
                        self.switches_dict[oids[i]] = 0
                    self.switches_dict[oids[i]] += 1
                ...
                if cat2 == 'TRANSFER':
                    if o not in self.last_match:
                        ...
                        self._append_to_indices(frameid, next(eid))
                        self._append_to_events(subcat, oids[i], hids[j], dists[i, j])
                        if oids[i] not in self.migrate_dict:
                            self.migrate_dict[oids[i]] = 0
                        self.migrate_dict[oids[i]] += 1
                    if oids[i] not in self.transfer_dict:
                        self.transfer_dict[oids[i]] = 0
                    self.transfer_dict[oids[i]] += 1
                    self._append_to_indices(frameid, next(eid))
